...
As of 2026-09-20, server persistence is deployed and verified after TLS fix `5938534`: cart → checkout → server order → reload/recovery → simulated tracking/history passed on the public URL. SQL confirms two smoke orders, two distinct idempotency keys and one confirmed event per order. The restricted `napoli_api` role accesses only Napoli tables in the existing Free project; its credential exists only as the Napoli Vercel Production Secret. No paid resources, real payment, account sync or backend courier tracking is claimed. Prior local-demo evidence above is historical; current evidence is in `docs/RELEASE.md`.
