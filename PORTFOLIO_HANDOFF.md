...
As of 2026-09-18, server mode is configured and deployed (commit `268b3f0`, Vercel Ready), using a restricted `napoli_api` database login in a separate schema of the existing Free project. Its credential exists only as the Napoli Vercel Production Secret. Runtime connectivity and the full hosted journey are **not yet verified**: public API/browser checks encountered Security Checkpoint / browser approval HTTP 403. Prior demo-mode evidence above is historical, not proof of this new backend deployment. No cross-device/account claim is made; see `docs/RELEASE.md`.
